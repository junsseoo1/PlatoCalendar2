...
class UserData {
  ...
  static int _firstDayOfWeek = 7;

  /// 완료된 일정 표시 여부
  static bool _showFinished = false;

  /// CalendarType : split(달력, 스케줄 분리 표시), integrated(한 페이지에 표시)
  static CalendarType _calendarType = CalendarType.integrated;
  ...
  static DateTime _lastSyncTime = DateTime(1999);
  static String _lastSyncInfo = "로그인이 필요합니다";

  /// 학생지원 시스템 마지막 동기화 날짜
  static int _oneStopLastSyncDay = 0;

  /// 마지막 notification 날짜
  static int _notificationDay = 0;
  ...
  static Set<String> subjectCodeThisSemester = {"전체"};
  ...
  /// Google Token 저장 여부.
  static bool isSaveGoogleToken = false;

  /// Google Login 직후 최초 앱실행.
  ///
  /// True일 경우 Google Calendar 전체 동기화 필요.
  static bool _googleFirstLogin = false;
  ...
  /// 알림 시간(분 단위, 예: 60, 120, 180, 240)
  static int _notificationBeforeMinutes = 60; // 기본값 60분
  ...
  static int get notificationBeforeMinutes {
    return writeDatabase.userDataBox.get('notificationBeforeMinutes',
        defaultValue: _notificationBeforeMinutes);
  }

  static set notificationBeforeMinutes(int value) {
    _notificationBeforeMinutes = value;
    writeDatabase.userDataBox.put('notificationBeforeMinutes', value);
  }

  /// 알림 시간 설정 및 알림 재예약
  static Future<void> setNotificationBeforeMinutes(int minutes) async {
    notificationBeforeMinutes = minutes;
    await Notify.updateEventReminderNotifications(); // 알림 재예약
    debugPrint('알림 시간 설정 완료: $minutes 분');
  }

  /// UserData 초기화 메서드
  static Future<void> initialize() async {
    // Hive 박스에서 데이터 로드
    await Database.init(); // Database 초기화
    if (!writeDatabase.userDataBox.containsKey('notificationBeforeMinutes')) {
      await writeDatabase.userDataBox.put('notificationBeforeMinutes', 60);
    }
    _notificationBeforeMinutes = writeDatabase.userDataBox.get(
        'notificationBeforeMinutes',
        defaultValue: 60);
    debugPrint(
        'UserData 초기화 완료: notificationBeforeMinutes = $_notificationBeforeMinutes');
  }
  ...
  static set themeMode(ThemeMode? newValue) {
    ...
    writeDatabase.userDataBox.put('themeMode', _themeMode);
  }
  ...
  static String getAllData() {
    ...
    result += "notificationBeforeMinutes : " +
        _notificationBeforeMinutes.toString() +
        '\n';
